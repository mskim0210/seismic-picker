...
9. [출력 형식](#9-출력-형식)
10. [Python API 사용법](#10-python-api-사용법)
11. [문제 해결](#11-문제-해결)
...
TPhaseNet은 PhaseNet의 U-Net 구조에 Transformer 블록을 결합한 모델입니다.

- **입력**: `(B, 3, 6000)` — 3성분(Z, N, E) 지진파형, 60초 @ 100Hz
- **출력**: `(B, 3, 6000)` — 각 시점별 [Noise, P파, S파] 확률 (softmax)
- **파라미터**: 약 517K (depth=5, stride=2, filters_root=8)

### 성능 (STEAD 테스트셋, 127,652 traces)

| Phase | Precision | Recall | F1 | Residual |
|-------|-----------|--------|------|----------|
| P-wave | 0.9708 | 0.9951 | **0.9828** | -0.007 ± 0.039 sec |
| S-wave | 0.9569 | 0.9738 | **0.9653** | +0.004 ± 0.111 sec |
...
- **GPU**: NVIDIA GPU 권장 (8GB+ VRAM), CPU에서도 동작
...
STEAD 데이터는 `source_id`의 md5 해시 기반으로 자동 분할됩니다 (PYTHONHASHSEED에 무관하게 재현 가능):

| 분할 | 비율 | 실제 수 | 용도 |
|------|------|---------|------|
| Train | 80% | 1,011,324 | 모델 학습 |
| Validation | 10% | 126,681 | 학습 중 성능 모니터링, early stopping |
| Test | 10% | 127,652 | 최종 평가 |

noise 트레이스(NaN source_id)는 `trace_name` 해시로 fallback하여 모든 분할에 균등 분배됩니다.
...
============================================================
  ...
  Threshold: 0.3
  Tolerance: 0.5s (50 samples)
  Total samples: 127,652
  Throughput: 5,432.1 samples/sec

  Phase     Precision     Recall         F1      TP     FP     FN
  ----------------------------------------------------------
  P            0.9708     0.9951     0.9828  103643   3116    507
  S            0.9569     0.9738     0.9653  101422   4563   2728

  Arrival Time Residuals:
    P-wave: -0.0071 ± 0.0391 sec
    S-wave: +0.0043 ± 0.1108 sec
============================================================
```

### 5.2 Threshold Sweep

여러 threshold에서 성능 변화를 분석합니다:
...
| Threshold | P-F1 | S-F1 | P-Precision | P-Recall |
|-----------|------|------|-------------|----------|
| 0.1 | 0.916 | 0.863 | 0.846 | 0.998 |
| 0.3 | 0.952 | 0.921 | 0.907 | 0.998 |
| 0.5 | 0.966 | 0.951 | 0.935 | 0.998 |
| 0.7 | 0.978 | 0.961 | 0.960 | 0.996 |
| **0.8** | **0.983** | **0.965** | **0.971** | **0.995** |

최적 threshold: **0.8** (precision과 recall의 균형점)

### 5.3 벤치마크 비교

TPhaseNet과 SeisBench 사전학습 모델을 동일 데이터셋에서 비교합니다:
...
| `--csv` | STEAD merged.csv 경로 | (필수) |
| `--hdf5` | STEAD merged.hdf5 경로 | (필수) |
| `--config`, `-c` | 설정 YAML 경로 | 체크포인트 내장 설정 |
| `--split` | 평가 분할 (`test` 또는 `val`) | `test` |
| `--batch-size` | 배치 크기 | `64` |
| `--device` | 연산 장치 | 자동 선택 |
| `--threshold` | pick 확률 임계값 | `0.3` |
| `--tolerance` | 허용 오차 (초) | `0.5` |
| `--sweep-thresholds` | 여러 threshold에서 분석 | 비활성 |
| `--max-samples` | 최대 샘플 수 제한 | 전체 |
| `--output-dir` | 결과 저장 디렉토리 | None |

---

## 6. 설정 파일

### 6.1 기본 설정 (`config/default.yaml`)
...
모든 스크립트에서 공유하는 기본 설정은 `config/defaults.py`에 정의되어 있습니다. 체크포인트에 저장된 config가 있으면 체크포인트 config가 우선합니다.

### 6.4 설정 우선순위

CLI 인수 > YAML 설정 파일 > 체크포인트 내장 config > `config/defaults.py` 기본값
...
Encoder (4-level 다운샘플링)
├── Input Conv: 3 → 8 채널
├── Level 0: DownBlock (8 → 8)
├── Level 1: DownBlock (8 → 16)
├── Level 2: DownBlock (16 → 32)
├── Level 3: DownBlock (32 → 64) + Transformer     ← transformer_start_level
└── Bottleneck: Conv (64 → 128) + Transformer

Decoder (4-level 업샘플링)
├── Level 3: UpBlock (128 → 64) + Skip Connection
├── Level 2: UpBlock (64 → 32) + Skip Connection
├── Level 1: UpBlock (32 → 16) + Skip Connection
├── Level 0: UpBlock (16 → 8) + Skip Connection
└── Output: 1x1 Conv → Softmax (3 classes)

출력: (B, 3, 6000) — [Noise, P, S] 확률
```

총 파라미터: **517,691** (depth=5, stride=2, filters_root=8)

### 7.2 핵심 블록

#### DownBlock
```
Input → Conv1d → BatchNorm → ReLU → Conv1d(stride=2) → BatchNorm → ReLU
                                                                      ↓
                                                              (skip, downsampled)
```
...
#### TransformerBlock
- Sinusoidal Positional Encoding (동적 길이 지원)
- Multi-head Self-Attention (4 heads, bottleneck에서 8 heads)
- Feed-Forward Network (GELU 활성화)
- Residual Connection + LayerNorm
- Level 3 이상의 인코더에만 적용 (저해상도 특징에서 전역 관계 학습)

Transformer 블록은 `nn.ModuleDict`로 관리되어, 적용되는 레벨만 모듈로 등록됩니다.
...
공통 로직은 `_load_and_align()` 헬퍼 함수에서 처리하며, `load_mseed()`(고정 길이)와 `load_mseed_stream()`(가변 길이) 모두 이를 공유합니다.
...
- **분할**: `hashlib.md5` 기반 결정적 분할 (PYTHONHASHSEED 무관, 재현 가능)
  - NaN `source_id`(노이즈 트레이스)는 `trace_name` 해시로 fallback
...
print(f"Parameters: {model.count_parameters():,}")  # 517,691
...
### 10.2 SeismicPicker (End-to-End 추론)
...
# (3, N) 형태의 파형
waveform = np.random.randn(3, 6000).astype(np.float64)

# 전처리 적용 (demean → detrend → bandpass → normalize)
config = {
    "filter": {"enabled": True, "freq_min": 0.5, "freq_max": 45.0, "corners": 4},
    "normalize": {"method": "std", "epsilon": 1e-8},
}
processed = preprocess(waveform, sampling_rate=100.0, config=config)
```
...
- **pick이 너무 많을 때**: threshold 높이기 → `--threshold 0.5` 또는 `0.8`
- **pick이 너무 적을 때**: threshold 낮추기 → `--threshold 0.1` 또는 `0.2`
- 설정 파일에서 `min_prominence`도 조정 가능
- threshold sweep으로 최적값 탐색: `python -m scripts.evaluate --sweep-thresholds`

### 학습 평가 메트릭

| 메트릭 | 설명 | 달성 결과 |
|--------|------|-----------|
| P-F1 | P파 검출 F1 스코어 | **0.983** |
| S-F1 | S파 검출 F1 스코어 | **0.965** |
| p_mean_residual_sec | P파 도착 시각 오차 평균 (초) | **-0.007** |
| s_mean_residual_sec | S파 도착 시각 오차 평균 (초) | **+0.004** |
